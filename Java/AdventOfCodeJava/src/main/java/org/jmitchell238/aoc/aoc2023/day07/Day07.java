  ...
  private Map<Character, Integer> countOccurrences(char[] hand) {
    ...
    if (getIsPart1() || !cardCountMap.containsKey('J')) {
      ...
      if (cardCountMap.containsKey('J')) {
        ...
        int highestCardCount = cardCountMap.values().stream().max(Integer::compare).get();
        char highestCardCountKey;
        if (highestCardCount == 1) {
          highestCardCountKey = cardCountMap.keySet().stream().max(Comparator.comparingInt(cardValues::get)).get();
          cardCountMap.put(highestCardCountKey, cardCountMap.get(highestCardCountKey) + jokerCount);
        } else {
          highestCardCountKey = cardCountMap.entrySet().stream().filter(entry -> entry.getValue() == highestCardCount).findFirst().get().getKey();
          cardCountMap.put(highestCardCountKey, cardCountMap.get(highestCardCountKey) + jokerCount);
        }
      }
    }
